...
def _accumulate_predictions_from_multiple_gpus(predictions_per_gpu):
    ...
    if len(image_ids) != image_ids[-1] + 1:
        ...
        logger.warning(
            "Number of train that were gathered from multiple processes is not "
            "a contiguous set. Some train might be missing from the evaluation"
        )
...
def inference(model, data_loader, dataset_name, device, output_folder=None, use_cached=False, **kwargs):
    ...
    logger = logging.getLogger("SSD.inference")
    logger.info("Evaluating {} dataset({} train):".format(dataset_name, len(dataset)))
