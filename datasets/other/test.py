...
parser.add_argument('--root_dir', default='./data',type=str, help="root path of train and labels, include ./train and ./labels and classes.txt")
parser.add_argument('--save_path', type=str,default='./instances_train.json', help="if not split the dataset, give a path to a json file")
...
def yolo2coco(arg):
    ...
    originImagesDir = os.path.join(root_path, 'train')
    ...
    if arg.random_split or arg.split_by_file:
        # 用于保存所有数据的图片信息和标注信息
        train_dataset = {'categories': [], 'annotations': [], 'train': []}
        val_dataset = {'categories': [], 'annotations': [], 'train': []}
        test_dataset = {'categories': [], 'annotations': [], 'train': []}
        ...
        dataset = {'categories': [], 'annotations': [], 'train': []}
        for i, cls in enumerate(classes, 0):
    ...
    for k, index in enumerate(tqdm(indexes)):
        # 支持 png jpg 格式的图片。
        txtFile = index.replace('train','txt').replace('.jpg','.txt').replace('.png','.txt')
        # 读取图像的宽和高
        print('\n')
        print(index)
        ...
        # 添加图像的信息
        dataset['train'].append({'file_name': index,
                                    'id': k,
                                    'width': width,
                                    'height': height})
